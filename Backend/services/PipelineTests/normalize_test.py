...
if __name__ == "__main__":
    ...
    # -------------------------------
    dataset_path = r"C:\Users\John Carlo\emoticoach\emoticoach\Backend\services\PipelineTests\normal.json"
    try:
    ...
    if 'text' not in df.columns or 'emotion' not in df.columns:
        raise ValueError("JSON must have 'text' and 'emotion' keys")

    tagalog_texts = df['text'].tolist()
    dataset_emotions = df['emotion'].tolist()

    print(f"Loaded {len(tagalog_texts)} synthetic samples for evaluation.")
    ...
    try:
        ...
        # -------------------------------
        output_path = r"C:\Users\John Carlo\emoticoach\emoticoach\Backend\services\PipelineTests\translated_output.json"
        analysis_data = results.get("analysis_data", [])

        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(analysis_data, f, indent=4, ensure_ascii=False)

        print(f"✅ Translated dataset with analysis saved to {output_path}")
