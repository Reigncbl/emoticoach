...
class ReadingContentController {
  ...
  Future<List<ChapterPage>> fetchAllPages(String readingsId) async {
    ...
    final url = Uri.parse('$baseUrl/book/$readingsId/all-blocks');
  ...
  Future<ChapterPage> fetchPage(String readingsId, int pageNumber) async {
    ...
    print('fetchPage: Requested page $pageNumber, total pages: ${allPages.length}');
    
    if (pageNumber <= 0 || pageNumber > allPages.length) {
      throw Exception('Page $pageNumber not found (total pages: ${allPages.length})');
    }
    
    final page = allPages[pageNumber - 1]; // Convert to 0-based index
    print('fetchPage: Returning page ${page.chapterNumber} with ${page.blocks.length} blocks');
  ...
  List<ChapterPage> _organizeBlocksIntoPages(List<dynamic> allBlocks, String bookTitle) {
    List<ChapterPage> pages = [];
    List<ReadingBlock> currentPageBlocks = [];
    String currentChapterTitle = "";
    int currentPageNumber = 0;
    
    print('_organizeBlocksIntoPages: Processing ${allBlocks.length} total blocks');
    
    // Convert all blocks to ReadingBlock objects first
    List<ReadingBlock> blocks = allBlocks
        .map((blockJson) => ReadingBlock.fromJson(blockJson as Map<String, dynamic>))
        .toList();
    ...
    for (ReadingBlock block in blocks) {
      if (block.blockType.toLowerCase() == 'chapter') {
        ...
        if (currentPageBlocks.isNotEmpty) {
          print('_organizeBlocksIntoPages: Creating page $currentPageNumber with ${currentPageBlocks.length} blocks');
          pages.add(ChapterPage(
            title: bookTitle,
            chapterTitle: currentChapterTitle.isNotEmpty ? currentChapterTitle : "Chapter $currentPageNumber",
        ...
        print('_organizeBlocksIntoPages: Started new page $currentPageNumber: "$currentChapterTitle"');
      } else {
    ...
    if (currentPageBlocks.isNotEmpty) {
      print('_organizeBlocksIntoPages: Creating final page $currentPageNumber with ${currentPageBlocks.length} blocks');
      pages.add(ChapterPage(
        title: bookTitle,
        chapterTitle: currentChapterTitle.isNotEmpty ? currentChapterTitle : "Chapter $currentPageNumber",
        chapterNumber: currentPageNumber,
    ...
    if (pages.isEmpty && blocks.isNotEmpty) {
      print('_organizeBlocksIntoPages: No chapters found, creating single page with all ${blocks.length} blocks');
      pages.add(ChapterPage(
        title: bookTitle,
  ...
  Reading convertChapterToReading(ChapterPage chapter) {
    ...
    return Reading(
      ...
      imageUrl: chapter.blocks
              .firstWhere((block) => block.imageUrl != null && block.imageUrl!.isNotEmpty,
                  orElse: () => ReadingBlock(
                      blockId: 0,
                      readingsId: '',
                      orderIndex: 0,
  ...
  Future<Reading> fetchReadingContent(String readingsId, int chapterNumber) async {
...
class ReadingProgressController {
  ...
  Future<ReadingProgress?> fetchProgress(String mobileNumber, String readingsId) async {
    ...
    final url = Uri.parse('$baseUrl/progress/$encodedMobile/$encodedReadingsId');

    print('Fetching progress for mobile: $mobileNumber, readingsId: $readingsId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('No progress found for mobile $mobileNumber and reading $readingsId');
        return null;
      } else {
  ...
  Future<Map<String, ReadingProgress>> fetchAllProgress(String mobileNumber) async {
    final encodedMobile = Uri.encodeComponent(mobileNumber);
    final url = Uri.parse('$baseUrl/progress-bulk/$encodedMobile');
  ...
  Future<List<ReadingWithProgress>> fetchReadingsWithProgress(String mobileNumber, List<Reading> readings) async {
    ...
    for (Reading reading in readings) {
      try {
        ...
        readingsWithProgress.add(ReadingWithProgress(
          reading: reading,
          progress: progress,
        ));
      } catch (e) {
  ...
  Future<List<ReadingWithProgress>> fetchReadingsWithProgressEfficient(String mobileNumber, List<Reading> readings) async {
    List<ReadingWithProgress> readingsWithProgress = [];
